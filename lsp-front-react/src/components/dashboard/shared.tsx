...
export function ProcessRow({ pid, name, user, cpu, memory, status }: { pid: string; name: string; user: string; cpu: number; memory: number; status: string }) {
  return (
...
export function StorageItem({ name, total, used, type }: { name: string; total: number; used: number; type: string }) {
...
export function AlertItem({ title, time, description, type }: { title: string; time: string; description: string; type: "info" | "warning" | "error" | "success" | "update" }) {
  const getTypeStyles = () => {
    switch (type) {
      case "info":
...
export function CommunicationItem({ sender, time, message, avatar, unread }: { sender: string; time: string; message: string; avatar: string; unread?: boolean }) {
  return (
