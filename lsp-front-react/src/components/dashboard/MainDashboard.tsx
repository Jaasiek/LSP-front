...
export function MainDashboard() {
  // Calculate real statistics from vehicle data
  const totalVehicles = table_data.length;
  const averageOdometer = Math.round(table_data.reduce((sum, v) => sum + v.odometer, 0) / totalVehicles);
  const activeRoutes = new Set(table_data.map(v => v.route)).size;
  const vehiclesNeedingService = table_data.filter(v => v.odometer > 450000).length;
  
  // Calculate fleet health percentage (based on mileage)
  const fleetHealth = Math.round(100 - (averageOdometer / 600000) * 100);
              ...
              <TabsContent value="vehicles" className="mt-0">
                <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 overflow-hidden">
                  <Table>
                    ...
                    <TableBody>
                      {table_data.slice(0, 8).map((vehicle) => (
              ...
              <TabsContent value="routes" className="mt-0">
                <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Array.from(new Set(table_data.map(v => v.route))).slice(0, 8).map((route) => {
                      const vehiclesOnRoute = table_data.filter(v => v.route === route);
                      return (
                        <div key={route} className="bg-slate-700/30 rounded-lg p-4 border border-slate-600/50">
                          ...
                          <div className="text-xs text-slate-400">
                            Średni przebieg: {Math.round(vehiclesOnRoute.reduce((s, v) => s + v.odometer, 0) / vehiclesOnRoute.length).toLocaleString()} km
                          </div>
                        </div>
                      );
                    })}
