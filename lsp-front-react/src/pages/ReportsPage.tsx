...
export default function ReportsPage() {
  const totalVehicles = table_data.length;
  const totalMileage = table_data.reduce((sum, v) => sum + v.odometer, 0);
  const averageMileage = Math.round(totalMileage / totalVehicles);
  const activeRoutes = new Set(table_data.map(v => v.route)).size;
  
  // Calculate statistics by brand
  const brandStats = table_data.reduce((acc: any, vehicle) => {
    if (!acc[vehicle.make]) {
      acc[vehicle.make] = { count: 0, totalMileage: 0 };
    }
    acc[vehicle.make].count += 1;
    acc[vehicle.make].totalMileage += vehicle.odometer;
    return acc;
  }, {});
  ...
  return (
    <div className="grid gap-6">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/50">
              ...
              <div className="text-3xl font-bold text-purple-400">{(totalMileage / 1000000).toFixed(1)}M</div>
              <p className="text-xs text-slate-500 mt-1">{totalMileage.toLocaleString()} km</p>
            </div>
            ...
            <div className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/50">
              ...
              <div className="text-3xl font-bold text-blue-400">{(averageMileage / 1000).toFixed(0)}k</div>
              <p className="text-xs text-slate-500 mt-1">{averageMileage.toLocaleString()} km</p>
            </div>
      ...
      <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="text-slate-100 text-base">Statystyki według Marki</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {Object.entries(brandStats).map(([brand, stats]: [string, any]) => (
              <div key={brand} className="bg-slate-800/30 rounded-lg p-4 border border-slate-700/50">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-purple-400 font-medium">
                      {Math.round(stats.totalMileage / stats.count).toLocaleString()} km
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
      ...
      <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="text-slate-100 text-base">Podsumowanie Miesięczne</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="bg-slate-800/30 rounded-lg p-6 border border-slate-700/50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-slate-300 font-medium mb-3">Koszty Operacyjne</h4>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Paliwo (szacowane):</span>
                    <span className="text-slate-200">~{(totalVehicles * 3500).toLocaleString()} zł</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Serwis i naprawy:</span>
                    <span className="text-slate-200">~{(totalVehicles * 1200).toLocaleString()} zł</span>
                  </div>
                  <div className="flex justify-between text-sm">
              ...
              <div>
                <h4 className="text-slate-300 font-medium mb-3">Wydajność Floty</h4>
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Czas pracy (h):</span>
                    <span className="text-slate-200">{(totalVehicles * 180).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-slate-400">Przejechane km:</span>
                    <span className="text-slate-200">{(totalVehicles * 4500).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm pt-2 border-t border-slate-600">
                    ...
                    <Badge className="bg-green-500/20 text-green-400 border-green-500/50">Wysoka</Badge>
                  </div>
                </div>
              </div>
            </div>
