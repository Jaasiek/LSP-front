...
interface EditModalProps {
  vehicle: typeof table_data[0];
  onClose: () => void;
  onSuccess: (message: string) => void;
}

function EditModal({ vehicle, onClose, onSuccess }: EditModalProps) {
  const [newRoute, setNewRoute] = useState(vehicle.route.toString());
      ...
      <div className="bg-slate-900 border-2 border-slate-700 rounded-lg p-6 max-w-md w-full mx-4 relative">
        ...
        <div className="bg-slate-800/50 rounded-lg p-4 mb-6 border border-slate-700/50">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-slate-200 font-medium">{vehicle.make}</span>
            <LicencePlate licenceNumber={vehicle.licencePlate} />
          </div>
          <p className="text-sm text-slate-300">
            Aktualne przypisanie: <span className="font-bold text-cyan-400">Trasa {vehicle.route}</span>
          </p>
        </div>
...
export default function VehicleAllocationPage() {
  const [editingVehicle, setEditingVehicle] = useState<typeof table_data[0] | null>(null);
