...
export default function UpcomingServicesPage() {
  // Vehicles needing service (high mileage)
  const vehiclesNeedingService = table_data.filter(v => v.odometer > 400000);
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-slate-700/50 hover:bg-slate-800/30">
                ...
                <TableHead className="text-slate-300">Szacowany Serwis</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {vehiclesNeedingService.map((vehicle) => {
                const daysUntilService = Math.max(0, Math.floor((500000 - vehicle.odometer) / 200));
                const priority = vehicle.odometer > 480000 ? "Wysoki" : vehicle.odometer > 450000 ? "Średni" : "Niski";
                  ...
                  <TableRow key={vehicle.id} className="border-slate-700/30 hover:bg-slate-700/30">
                    ...
                    <TableCell className="text-slate-300">
                      <div className="flex items-center gap-2">
                        {vehicle.odometer > 480000 && <AlertTriangle className="h-4 w-4 text-red-500" />}
        ...
        <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-red-400">
              {vehiclesNeedingService.filter(v => v.odometer > 480000).length}
            </div>
        ...
        <Card className="bg-slate-900/50 border-slate-700/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-3xl font-bold text-amber-400">
              {vehiclesNeedingService.filter(v => v.odometer > 450000 && v.odometer <= 480000).length}
            </div>
            <p className="text-sm text-slate-400 mt-1">pojazd(ów)</p>
          </CardContent>
        </Card>
